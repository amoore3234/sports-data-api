...
def generate_mlb_lineup() -> dict:
  ...
  salary_data_df.rename(columns={'Name': 'name', 'TeamAbbrev': 'team', 'Name + ID': 'name_id', 'Salary': 'salary', 'Game Info': 'game_info', 'Position': 'position'}, inplace=True)
  ...
  if is_pitcher_friendly_park:
    ...
    # batting_lineup_df = stats_util.drop_hitters_at_pitcher_friendly_ballpark(pitcher_lineup_df, batting_lineup_df)
  ...
  generate_optimal_lineup(salary_data_df, pitcher_lineup_df, batting_lineup_df,
                          is_hitter_friendly_park, is_pitcher_friendly_park, is_fanduel_lineup)
...
def get_missing_hitters(batting_lineup_df, salary_df) -> pd.DataFrame:
  ...
  for starting_position in starting_lineup_positions:
    ...
    position_splits.append(starting_position)
...
#   new_battig_lineup_df['salary'] = new_batting_lineup_df[salary]
...
def generate_starting_lineups(group_position_name, salary_df) -> pd.DataFrame:
  ...
  if group_position_name == 'hitters':
    ...
    return get_starters(salary_df, hitters_list, length_value=4, index=2)
  else:
    pitchers_list = list(data.get_starting_lineup()['Starting Lineup'])
    return get_starters(salary_df, pitchers_list, length_value=3, index=1)

def get_starters(salary_df, starting_list, length_value, index) -> pd.DataFrame:
  ...
  lineup_df = salary_df[salary_df['name'].str.contains(lookup_table)]
...
#   teams = set(batting_lineup_df['team_abbrev'])
#   seen = set()
#   pitcher_matchups = list(pitcher_lineup_df['team_abbrev'])
...
#       pitcher_lineup_df = pitcher_lineup_df.drop(pitcher_lineup_df[pitcher_lineup_df['team_abbrev'] == pitcher_away_team.get('away_team')].index)
...
def generate_optimal_lineup(salary_data_df, pitcher_lineup_df, batting_lineup_df,
                            is_hitter_friendly_park, is_pitcher_friendly_park, is_fanduel_lineup) -> list[dict]:
  """Lineup Generator.
...
def map_players_salary(salary_data_df, lineup_df, elite_players, column_name) -> pd.DataFrame:
  ...
  lineup_df['team_abbrev'] = elite_players[column_name].map(team_lookup)
  lineup_df = lineup_df.dropna()

  return lineup_df

def drop_hitters_against_pitchers(salary_data_df, pitcher_idx, pitcher_lineup_df, batting_lineup_df,
                                  player_salary, pitcher_indices) -> None:
  ...
  """
  game_matchups = get_list_of_team_game_matchups(salary_data_df['game_info'].unique())

  pitcher_matchup = pitcher_lineup_df.loc[pitcher_idx]['team']
  pitcher_home_team = next((home for home in game_matchups if home.get('home_team') == pitcher_matchup), None)
  pitcher_away_team = next((away for away in game_matchups if away.get('away_team') == pitcher_matchup), None)

  if pitcher_home_team:
    batters_away = pitcher_home_team.get('away_team')
  ...
  # Drop other starters and relieve pitchers on the same team from the dataset.
