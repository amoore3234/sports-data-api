...
def get_starters(salary_df, starting_list, length_value, index) -> pd.DataFrame:
  ...
  print(f"Starting lineup: {lineup_df}")
...
def drop_hitters_against_pitchers(salary_data_df, pitcher_idx, pitcher_lineup_df, batting_lineup_df,
  ...
  pitcher_matchup = pitcher_lineup_df.loc[pitcher_idx]['team_abbrev']
...
def filter_hitters_against_pitchers(team_matchup, batting_lineup_df):
  ...
  """
  batting_lineup_df.drop(batting_lineup_df[batting_lineup_df['team_abbrev'] == team_matchup].index, inplace=True)
